...
tidyverse: 8 packages
large data set -> usde data table 
...
## Tidy data {#tidy-data}
we have one observation for one row only.
...
d. `co2` is not tidy: to be tidy we would have to wrangle it to have three columns (year, month and value), then each co2 observation would have a row. -> correct
...
b.  `ChickWeight` is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one of the variables. -> True
...
c. `BOD` is tidy: each row is an observation with two values (time and demand) -> True
...
can also change column
murders: its a tidy data
tidyverse: it gives data in as data frame and out as data frame. Allows us to define the column names in the first data frame.
...
only show me the rows that the rate is less than 0.71
...
>>>>>>> eeb0ebb1aff1c49e96a3fb1dfad5168cfc95ed84
...
data pipe into select pipe into filter
```{r, eval=FALSE}
murders |> select(state, region, rate) |> filter(rate <= 0.71)
```
...
### `summarize` {#summarize}
summarize still gives back data frame
```{r}
library(dplyr)
library(dslabs)
data(heights)
...
summarize function take the data frame and take the mean and give it back. 
```{r}
s <- heights |> 
  filter(sex == "Female") |>
...
not the average rate of united states, need to account for the population
```{r}
...
### Multiple summaries
quantile shows you whatever quantile you want for a vector
```{r}
heights |> 
  filter(sex == "Female") |>
...
gives out a tibble -> special type of data frame
```{r}
...
heights |> 
  ...
  summarize(average = mean(height), standard_deviation = sd(height))
```
When we the group_by we are doing on is a vector, it will goes in the order we defined it originally. But if it is a character, then it will be in alphabetical order. 
```{r}
murders |> 
  group_by(region) |>
...
murders |> 
  group_by(region) |>
  summarize(quantile(rate, c(0.5, 0,1)))
```


## `pull`
extract a vector from a data frame. extract columns by names into a vector.
```{r}
